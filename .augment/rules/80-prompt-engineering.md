...
All paths are relative to the workspace root. The `./prompts/` directory is resolved from the current working directory, which should be the project root.
</context>
...
- options:
  ...
  - "Create throwaway (don't save)" - Generate the prompt content inline without writing to `./prompts/`
...
If "Create throwaway (don't save)"  generate the prompt content inline without writing to `./prompts/`, then ask: "Would you like to persist this prompt to ./prompts/ for reuse?" and, if the user confirms, follow the normal numbering flow to save it to `./prompts/`
</decision_gate>
...
Numbering behavior for prompt files:

- Numbers are zero-padded to three digits (001, 002, 003, etc.)
- When gaps exist (e.g., 001, 003), always choose the next number as the highest existing number + 1 (004 in this example)
- Use the Glob tool on `./prompts/*.md` to find the highest existing number before assigning a new one
  </pre_generation_analysis>
...
5. **Context Loading**: Only request file reading when the task explicitly requires understanding existing code. Use patterns like:
   - "Examine @Cargo.toml for dependencies" (when adding new packages)
   - "Review @src/\* for existing patterns" (when modifying code)
...
<action>
If user chooses #1, invoke via SlashCommand tool: `auggie command run-prompt 005`
</action>
</single_prompt_scenario>
...
<actions>
If user chooses #1, invoke via SlashCommand tool: `auggie command run-prompt 005 006 007 --parallel`
If user chooses #2, invoke via SlashCommand tool: `auggie command run-prompt 005 006 007 --sequential`
</actions>
</parallel_scenario>
...
<actions>
If user chooses #1, invoke via SlashCommand tool: `auggie command run-prompt 005 006 007 --sequential`
If user chooses #2, invoke via SlashCommand tool: `auggie command run-prompt 005`
</actions>
</sequential_scenario>
...
- Use the SlashCommand tool to invoke `auggie command run-prompt` when user makes their choice
  </meta_instructions>
